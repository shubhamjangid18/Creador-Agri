...
const MarqueeRow = ({ images, direction, speed = 32, onOpen }) => (
  <div className="agri-marquee">
    <div
      className={`agri-track ${direction === "right" ? "agri-track-right" : "agri-track-left"}`}
      style={{ "--agri-speed": `${speed}s` }}
    >
      ...
      <div className="agri-set" aria-hidden="true">
        {images.map((image, index) => (
          <button
            className="agri-card"
...
const ProductViewer = ({ isOpen, currentIndex, onClose, onNext, onPrevious }) => {
  useEffect(() => {
  ...
  return (
    ...
    >
      <button className="agri-viewer-button agri-viewer-button-left" onClick={onPrevious} aria-label="Previous product" type="button">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 5L8 12L15 19"></path></svg>
      </button>
      ...
      <button className="agri-viewer-button agri-viewer-button-right" onClick={onNext} aria-label="Next product" type="button">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 5L16 12L9 19"></path></svg>
      </button>

      <button className="agri-viewer-button agri-viewer-button-close" onClick={onClose} aria-label="Close" type="button">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M6 6L18 18"></path>
          <path d="M18 6L6 18"></path>
        </svg>
...
const Marquee = () => {
  ...
  const openInlineViewer = useCallback((image) => {
    const index = PRODUCTS.indexOf(image);
    setViewerIndex(index === -1 ? 0 : index);
    setViewerOpen(true);
  }, []);

  const closeInlineViewer = useCallback(() => setViewerOpen(false), []);

  const showNext = useCallback(
    () => setViewerIndex((prev) => (prev + 1) % PRODUCTS.length),
    []
  );

  const showPrevious = useCallback(
    () => setViewerIndex((prev) => (prev - 1 + PRODUCTS.length) % PRODUCTS.length),
    []
  );

  return (
    <section className="agri-marquee-section">
      <div className="agri-marquee-wrapper">
        <MarqueeRow images={ROWS[0]} direction="left"  speed={34} onOpen={openInlineViewer} />
        <MarqueeRow images={ROWS[1]} direction="right" speed={37} onOpen={openInlineViewer} />
        <MarqueeRow images={ROWS[2]} direction="left"  speed={40} onOpen={openInlineViewer} />
      </div>

      <div className="agri-view-wrapper">
        <button className="agri-view-all" onClick={openAllProducts} type="button">
